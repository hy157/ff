...
let energy = 5, gold = 0, wheat = 0, diamond = 0;
...
const rewards = [
  { type: "energy", amount: 1, icon: "assets/images/iconenergy.png", text: "+1 Enerji" },
  { type: "gold", amount: 5, icon: "assets/images/icongold.png", text: "+5 Altın" },
  { type: "wheat", amount: 3, icon: "assets/images/iconwheat.png", text: "+3 Buğday" },
  { type: "diamond", amount: 1, icon: "assets/images/icondiamond.png", text: "+1 Elmas" }
];
...
function nextStage() {
  if(fieldStage === 0) {
    ...
    rewards.forEach(r => {
      ...
      if(r.type === "diamond") diamond += r.amount;
    });
    updateInfo();
    showRewardAnimation();
    fieldStage = 4;
...
// Menü butonlarına örnek tıklama olayları (geliştirilebilir!)
document.getElementById("btnBarn").onclick = function() {
  alert("Çiftlik: Burada hayvanlar veya stokları görebilirsin! (Demo)");
}
document.getElementById("btnStore").onclick = function() {
  alert("Market: Buradan tohum veya ürün satın alabilirsin! (Demo)");
}
document.getElementById("btnMap").onclick = function() {
  alert("Harita: Farklı bir tarla alanı seçebilirsin! (Demo)");
}
document.getElementById("btnTasks").onclick = function() {
  alert("Görevler: Günlük görevler burada listelenir! (Demo)");
}
document.getElementById("btnSettings").onclick = function() {
  alert("Ayarlar: Oyunun ayarları burada! (Demo)");
}
